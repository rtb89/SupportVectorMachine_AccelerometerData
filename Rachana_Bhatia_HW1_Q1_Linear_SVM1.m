...
clc;
clear all;
%%Setting the path to the directory with the Input data set
...
for k = 1:10
    ...
    %% Fitting the Support vector machine with the fitcsvm command
     ...
     svm = svmtrain(dataTrain,labelsTrain,'autoscale','false','Method','SMO','SMO_Opts',smoopt,'Kernel_Function','linear');
%     

   %%Predicting the Training Class
   trainPredict= svmclassify(svm,dataTrain);
    ...
    testPredict = svmclassify(svm,dataTest);
